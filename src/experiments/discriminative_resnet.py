...
from layers.variational import BasicBlockVCL
from models.contrib import ResNetVCL, ConvVCL
...
def permuted_cifar10():
    ...
    summary_logdir = os.path.join("logs", "disc_resnet_p_cifar10", datetime.now().strftime('%b%d_%H-%M-%S'))
    ...
    for task in range(N_TASKS):
        run_task(
            ...
            device=device, lr=LR, save_as="disc_resnet_p_cifar10",
...
def split_cifar10(partial=False):
    ...
    summary_logdir = os.path.join("logs", "disc_resnet_s_cifar10", datetime.now().strftime('%b%d_%H-%M-%S'))
    writer = SummaryWriter(summary_logdir)
    ...
    for task_idx in range(N_TASKS):
        run_task(
            ...
            save_as=f"disc_resnet_s_cifar10_{datetime.now().strftime('%b%d_%H-%M-%S')}_coreset{CORESET_SIZE}", device=device, multiheaded=MULTIHEADED,
...
def split_cifar100(epochs=200):
    ...
    summary_logdir = os.path.join("logs", "disc_resnet_s_cifar100", datetime.now().strftime('%b%d_%H-%M-%S'))
    writer = SummaryWriter(summary_logdir)
    ...
    for task_idx in range(N_TASKS):
        run_task(
            ...
            test_data=cifar_test, test_task_ids=test_task_ids, coreset=coreset,
            task_idx=task_idx, epochs=EPOCHS, batch_size=BATCH_SIZE, lr=LR,
            save_as=f"disc_resnet_s_cifar100_{datetime.now().strftime('%b%d_%H-%M-%S')}_coreset{CORESET_SIZE}_epochs{EPOCHS}", device=device, multiheaded=MULTIHEADED,
...
def permuted_mnist():
    ...
    model = ResNetVCL(
        BasicBlockVCL, [2,2,2], num_tasks=(N_TASKS if MULTIHEADED else 1),
        num_classes_per_task=N_CLASSES, in_channels=1, initial_posterior_variance=INITIAL_POSTERIOR_VAR, device=device
    ).to(device)
    ...
    summary_logdir = os.path.join("logs", "disc_resnet_p_mnist", time)
    writer = SummaryWriter(summary_logdir)
    ...
    for task in range(N_TASKS):
        run_task(
            ...
            coreset=coreset, epochs=EPOCHS, batch_size=BATCH_SIZE,
            device=device, lr=LR, save_as=f"disc_resnet_p_mnist_{time}",
            multiheaded=MULTIHEADED, train_full_coreset=TRAIN_FULL_CORESET,
            summary_writer=writer
        )
...
def split_mnist():
    ...
    summary_logdir = os.path.join("logs", "disc_resnet_s_mnist", datetime.now().strftime('%b%d_%H-%M-%S'))
    writer = SummaryWriter(summary_logdir)
    ...
    for task_idx in range(N_TASKS):
        run_task(
            ...
            task_idx=task_idx, epochs=EPOCHS, batch_size=BATCH_SIZE, lr=LR,
            save_as=f"disc_resnet_s_mnist_{datetime.now().strftime('%b%d_%H-%M-%S')}", device=device, multiheaded=MULTIHEADED,
            y_transform=binarize_y, train_full_coreset=TRAIN_FULL_CORESET,
            summary_writer=writer
        )
...
def split_not_mnist():
    ...
    time = datetime.now().strftime('%b%d_%H-%M-%S')
    summary_logdir = os.path.join("logs", "disc_resnet_s_n_mnist", time)
    writer = SummaryWriter(summary_logdir)
    ...
    for task_idx in range(N_TASKS):
        run_task(
            ...
            batch_size=BATCH_SIZE, save_as=f"disc_resnet_s_n_mnist_{time}", device=device,
